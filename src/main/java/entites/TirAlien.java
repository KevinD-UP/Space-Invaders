...
public class TirAlien extends Entite{
    ...
    public boolean tirAlienAHauteurDeChateau(){
        if(this.yPos < Constantes.Y_POS_CHATEAU + Constantes.HAUTEUR_CHATEAU && this.yPos + this.hauteur > Constantes.Y_POS_CHATEAU){
            return true;
        }else{
            return false;
        }
    ...
    private int chateauProche(){
        ...
        while(numeroChateau == -1 && colonne < 4){
            colonne++;
            if(this.xPos + this.largeur - 1 > Constantes.MARGE_FENETRE + Constantes.X_POS_INIT_CHATEAU + colonne * (Constantes.LARGEUR_CHATEAU + Constantes.ECART_CHATEAU)
                    && this.xPos + 1 < Constantes.MARGE_FENETRE + Constantes.X_POS_INIT_CHATEAU + Constantes.LARGEUR_CHATEAU + colonne * (Constantes.LARGEUR_CHATEAU + Constantes.ECART_CHATEAU)){
    ...
    public void tirAlienDetruitChateau(Chateau tabChateaux[]){
        ...
        if(tab[0] != -1 ){
            ...
            && tabChateaux[tab[0]].trouveBriqueHaut(tabChateaux[tab[0]].trouveColonneChateau(tab[1])) != 27){
                tabChateaux[tab[0]].casseBriqueHaut(tab[1]);
                this.yPos = 700;
            }
        }
    }
